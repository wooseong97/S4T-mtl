...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, out_chans=3,
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, self.patch_embed.num_patches, embed_dim), requires_grad=False)  # fixed sin-cos embedding
        ...
        self.decoder_pos_embed = nn.Parameter(torch.zeros(1, self.patch_embed.num_patches, decoder_embed_dim), requires_grad=False)  # fixed sin-cos embedding
...
def mae_vit_tiny_patch16(**kwargs):
    ...
    model_kwargs = overwrite_kwargs(model_kwargs, **kwargs)
...
def overwrite_kwargs(dict, **kwargs):
    for key in kwargs.keys(): dict[key] = kwargs[key]
    return dict
