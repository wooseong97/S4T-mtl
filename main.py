...
from utils.common_config import get_train_dataset, get_transformations,\
                                ...
                                get_optimizer, get_model, get_criterion, load_pretrained_ckpt, dataset_config_TTT
from utils.logger import Logger
from utils.train_utils import train_phase_S4T, train_phase_S4T_ttt, tb_update_perf, init_point
...
def main():
    ...
    # Get model
    model = get_model(p)
    print("#"*10); print("Number of Param"); print(count_parameters(model)); print("#"*10)
    ...
    if DEBUG_FLAG and args.local_rank == 0:
        print("\nFirst Testing...")
        eval_test = test_phase(p, test_dataloader, model)
        print(eval_test)
    
    if 'val_seg' in p.keys(): dataset_config_TTT(p)
    
    if 'pretrained_ckpt' in p.keys():
        print('Use checkpoint {}'.format(p['pretrained_ckpt']))
        checkpoint = torch.load(p['pretrained_ckpt'], map_location='cpu')
        ckp_model = load_pretrained_ckpt(p, model, checkpoint['model'])
        model.load_state_dict(ckp_model)
        init = init_point(model, optimizer)
        
        # Initial Test
        if args.local_rank == 0 and iter_count==0:
    ...
    if args.run_mode != 'infer':
        for epoch in range(start_epoch, p['epochs']):
            ...
            if args.local_rank == 0:
                ...
                print('-'*10)
            if 'pretrained_ckpt' in p.keys():
                # Test-time training (target): single-batch episodic adaptation, then evaluate.
                end_signal, iter_count = train_phase_S4T_ttt(p, args, train_dataloader, test_dataloader, model, criterion, optimizer, scheduler, epoch, tb_writer_train, tb_writer_test, iter_count, init)
            else:
                # Source-domain pre-training.
                end_signal, iter_count = train_phase_S4T(p, args, train_dataloader, test_dataloader, model, criterion, optimizer, scheduler, epoch, tb_writer_train, tb_writer_test, iter_count)
            if end_signal: break
