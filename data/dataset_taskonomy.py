...
db_root_taskonomy = "../datasets/"

all_tasks = ['class_object', 'class_scene', 'depth_euclidean', 'depth_zbuffer', 'keypoints2d', 'edge_occlusion', 'edge_texture', 'keypoints3d', 'normal', 'principal_curvature', 'reshading', 'rgb', 'segment_semantic', 'segment_unsup2d', 'segment_unsup25d']
...
# segment_semantic
# current_scale['segment_semantic'] = [0.0, 255.0]
...
class TaskonomyDataset(data.Dataset):
    ...
    def __init__(self, img_types, data_dir=os.path.join(db_root_taskonomy, "taskonomy/data"), 
        split='medium', partition='train', transform=None, resize_scale=None, crop_size=None, fliplr=False, val_seg='nyu'):
        ...
        for scene in self.scene_list:
            ...
            # #################
            # img_types.insert(0, img_types.pop(img_types.index('segment_semantic')))
            # non_valid = []
            # #################
            for img_type in img_types:
                ...
                #         np_img = np.array(img)
                #         if np.mean(np_img) == 255:
                #             non_valid.append(i_image)
                # #################
        ...
        if val_seg == 'NYUD':
            self.do_nyu = True
        elif val_seg == 'PASCALContext':
            self.do_voc = True
        self.to_nyu = [14, 7, 8, 4, 6, 13, 3]
        self.to_voc = [7, 9, 2, 4, 5, 3]
    ...
    def __getitem__(self, index):
        ...
        for img_type in self.img_types:
            if img_type == 'class_scene' or img_type == 'class_object':
                ...
                elif 'segment_semantic' in img_type:
                    # output[img_type] = preprocess_segment_semantic(np.array(output[img_type]))
                    output[img_type] = np.array(output[img_type])
                    output[img_type] = self.filter_semseg(output[img_type])
                else:
        ...
        output['image'] = output['rgb']
        return output

    def filter_semseg(self, _semseg):
        _semseg_ref = np.copy(_semseg)
        if self.do_voc:
            mask = np.isin(_semseg, self.to_voc)
            _semseg[~mask] = 255
            for new_idx, idx in enumerate(self.to_voc):
                _semseg[_semseg_ref==idx] = new_idx
        elif self.do_nyu:
            mask = np.isin(_semseg, self.to_nyu)
            _semseg[~mask] = 255            
            for new_idx, idx in enumerate(self.to_nyu):
                _semseg[_semseg_ref==idx] = new_idx
        else:
            pass
        return _semseg
