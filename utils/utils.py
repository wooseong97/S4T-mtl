...
def get_output(output, task):
    ...
    elif task in {'segment_semantic'}:
        _, output = torch.max(output, dim=1)

    elif task in {'depth_euclidean', 'depth_zbuffer'}:
        output = output.squeeze()

    elif task in {'edge_occlusion', 'edge_texture', 'keypoints2d', 'keypoints3d', 'normal', 'principal_curvature', 'reshading', 'segment_unsup2d', 'segment_unsup25d'}:
