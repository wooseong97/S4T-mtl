...
def get_single_task_meter(p, database, task):
    ...
    #     return DepthMeter(ignore_index=p.ignore_index) 
        ...
        from evaluation.eval_scum import ScumMeter
        # return ScumMeter(ignore_index=p.ignore_index, task=task)
        return ScumMeter(ignore_index=p.ignore_index, task=task)
    ...
    elif task in ['edge', 'edge_occlusion', 'edge_texture']:
        from evaluation.eval_scum import ScumMeter
        return ScumMeter(ignore_index=p.ignore_index, task=task)
    else:
        from evaluation.eval_scum import ScumMeter
