...
TASK_CATEGORY_NAMES = ['bottle', 'chair', 'couch', 'plant',
                        'bed', 'd.table', 'toilet', 'tv', 'microw', 
                        'oven', 'toaster', 'sink', 'fridge', 'book',
                        'clock', 'vase']

NYU_VOC = ['person', 'sofa', 'chair', 'table']
VOC_TASK = ['table', 'tv', 'toilet', 'sofa', 'pottedplant', 'chair']
# NYU_TASK = ['chair', 'bed', 'sink', 'refridgerator', 'toilet', 'table', 'sofa']
NYU_TASK = ['refridgerator', 'table', 'toilet', 'sofa', 'bed', 'sink', 'chair']

class SemsegMeter(object):
    def __init__(self, database, val_seg, ignore_idx=255):
        ...
        if database == 'PASCALContext':
            ...
            if val_seg == 'PASCALContext':
                ...
                n_classes = 16
                cat_names = TASK_CATEGORY_NAMES
                has_bg = False
        
        else:
            raise NotImplementedError
        
        if val_seg in ['NYUD', 'PASCALContext', 'Taskonomy']:
